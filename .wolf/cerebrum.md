...
- **Always bump the version and rebuild PCM artifacts on every push.** Skipping this left PCM showing no update, which caused confusion. After every code change: bump `metadata.json` version, run `python tools/build_pcm_release.py`, include packages.json / repository.json / zip in the commit.
...
- [2026-04-30] When removing a field from `StitchSettings`, grep every method in `via_stitcher.py` for references to that field — `_group_name()`, `_group_name()` etc. can silently fail inside `try/except` blocks and take down an unrelated feature (grouping) with no visible error. Always search the whole file before committing a settings removal.
